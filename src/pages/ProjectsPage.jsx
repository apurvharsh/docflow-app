...
import { Search, Clock3, Files, Sparkles, Sun, Moon } from 'lucide-react';
  ...
  return (
    <div className="flex-1 px-5 py-8 lg:px-10 max-w-[1500px] mx-auto w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-9">
        {[
          { label: 'Active projects', value: projects.length, icon: Files },
          { label: 'Documents in flow', value: projects.reduce((total, project) => total + (project.document_count || 0), 0), icon: Clock3 },
          { label: 'AI-ready workspace', value: '100%', icon: Sparkles },
        ].map(({ label, value, icon: Icon }) => (
          <div key={label} className="bg-surface border border-border rounded-xl px-5 py-4 flex items-center gap-4">
            <span className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center"><Icon size={18} /></span>
            <div><p className="text-2xl font-bold text-gray-100">{value}</p><p className="text-xs text-gray-500">{label}</p></div>
          </div>
        ))}
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            {filteredProjects.map((project) => (
              (() => {
                const projectMember = project.members?.find((member) => member.user_id === user?.user_id);
                return (
                  <ProjectCard
                    key={project.project_id}
                    project={project}
                    role={user?.is_org_admin ? 'org_admin' : (user?.project_roles?.[project.project_id] || projectMember?.role)}
                    teamName={user?.team_name || projectMember?.team_name}
                  />
                );
              })()
            ))}
