...
def _command_response(message: str, state: dict) -> dict:
    ...
    if text in {"/draft", "/scan", "/rag", "/query"}:
        ...
        if state["active_agent"] == "rag":
            return {
                "response": "RAG Retrieval Agent is active. Ask a question to search the authorized project knowledge base.",
                "state": state,
            }
        return {
            "response": "General Query Agent is active. Ask a question to retrieve grounded answers from the project knowledge base.",
            "state": state,
        }
...
def handle_cli_message(message: str, state: dict | None = None) -> dict:
    ...
    if active in {"rag", "query"}:
        return {
            "response": (
                "The web workspace handles this agent with authenticated tenant/project scope. "
                "Open Studio > RAG Retrieval Agent or General Query Agent to search and ask questions."
            ),
            "state": state,
        }
