...
class GeneralQueryAgent:
    ...
    @staticmethod
    def build_project_context(project_id: str, documents: list[dict], stages: list[str]) -> str:
        """Build a compact inventory so Gemini can reason across the whole project."""
        gaps = GapDetectionAgent.detect_gaps(project_id, stages, documents)
        documents_by_stage = {stage: [] for stage in stages}
        for document in documents:
            stage = document.get("stage", "Unspecified")
            documents_by_stage.setdefault(stage, []).append(document)

        stage_inventory = []
        for stage in stages:
            stage_documents = documents_by_stage.get(stage, [])
            if stage_documents:
                entries = "; ".join(
                    f"{document.get('filename', 'Unnamed document')} "
                    f"({document.get('doc_type', 'unknown')}, "
                    f"{document.get('workflow_state', 'unknown')})"
                    for document in stage_documents
                )
            else:
                entries = "No documents uploaded"
            stage_inventory.append(f"- {stage}: {entries}")

        extra_stages = sorted(set(documents_by_stage) - set(stages))
        for stage in extra_stages:
            entries = "; ".join(
                f"{document.get('filename', 'Unnamed document')} "
                f"({document.get('doc_type', 'unknown')}, "
                f"{document.get('workflow_state', 'unknown')})"
                for document in documents_by_stage[stage]
            )
            stage_inventory.append(f"- {stage}: {entries}")

        missing = "\n".join(
            f"- {stage}: {', '.join(items)}"
            for stage, items in gaps["gaps_by_stage"].items()
        ) or "- No gaps detected from the configured stage checklist."
        return (
            f"Project ID: {project_id}\n"
            f"Stage coverage across the entire project:\n"
            f"{chr(10).join(stage_inventory) or '- No stages configured.'}\n\n"
            f"Documentation gaps ({gaps['total_gaps']}):\n{missing}"
        )
    ...
    def process_query(
        ...
        """
        if not hits and not project_context:
